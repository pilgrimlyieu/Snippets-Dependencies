...
def create_align_table(snip, align = 0):
...
def create_table(snip, align = 0):
    placeholders_string = snip.buffer[snip.line].strip()[-2:]
    rows = int(placeholders_string[0])
    columns = int(placeholders_string[1])
    snip.buffer[snip.line] = ""
    anon_snippet_title = "| "
    anon_snippet_delimiter = "|"
    for col in range(1, columns + 1):
        anon_snippet_title += "${" + str(col)  + ":head" + str(col)\
            + "} | "
        anon_snippet_delimiter += table_settings[align][0] + "-" + table_settings[align][1] + "|"
    anon_snippet_title += "\n"
    anon_snippet_delimiter += "\n"
    anon_snippet_body = ""
    for row in range(1, rows + 1):
        ...
        for col in range(1, columns + 1):
            placeholder = "R{0}C{1}".format(row, col)
            body_row += "${" + str(row * columns + col)  + ":" + placeholder\
                + "} | "
        body_row += "\n"
        anon_snippet_body += body_row
    anon_snippet_table = anon_snippet_title + anon_snippet_delimiter + anon_snippet_body
